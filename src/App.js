...
function App({ questions }) {
  ...
  const onNextClick = (data) => {
    if (quizStatus === 0) {
      // If on initial form, start quiz at question 0
    ...
    if (questionIndex === questions.length - 1) {
      ...
      const correctPoints = answerStatus
        ? correctAnswerCount + questions[questionIndex].points
        : correctAnswerCount;
      setQuizStatus(2);
      // Send email
  ...
  if (quizStatus === 0) {
    ...
    main = (
      <div className="quiz">
        ...
        <p>
          This is a short diagnostic test for us to know how profficient you are
          in reading, understanding, and speaking english.
        </p>
        ...
        <form onSubmit={handleSubmit(onNextClick)}>
          ...
          {errors.studentName && (
            <p role="alert">{errors.studentName?.message}</p>
          )}
          <label>
            Phone:
            <Input
          ...
          {errors.studentPhone && (
            <p role="alert">{errors.studentPhone?.message}</p>
          )}
          <label>
            Email:
            <Input
          ...
          {errors.studentEmail && (
            <p role="alert">{errors.studentEmail?.message}</p>
          )}
    ...
    main = (
      <Fragment>
        ...
        <p>
          ...
          you shortly to inquire about your placement.
        </p>
      </Fragment>
    );
  }

  return <div className="quiz">{main}</div>;
}
